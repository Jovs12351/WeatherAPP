...
function displayWeather(data) {
    ...
    temperature.textContent = `${Math.round(current.temp_c)}°C / ${Math.round(current.temp_f)}°F`;
    ...
    additionalInfo.innerHTML = `
        <div class="row">
            <div class="col-sm-6">
                <div class="additional-info-item">
                    ...
                    <span>Feels like: ${Math.round(current.feelslike_c)}°C / ${Math.round(current.feelslike_f)}°F</span>
                </div>
                <div class="additional-info-item">
            ...
            <div class="col-sm-6">
                ...
                <div class="additional-info-item">
                    ...
                    <span>Cloud cover: ${current.cloud}%</span>
                </div>
                <div class="additional-info-item">
        ...
        <div class="additional-info-item">
            ...
            <span>Air Quality (US EPA Index): ${getAirQualityDescription(current.air_quality['us-epa-index'])}</span>
        </div>
    `;
            ...
            ${forecastDays.map(day => `
                <div class="col-sm-4 mb-3">
                    <div class="forecast-day">
                        <p class="mb-2">${new Date(day.date).toLocaleDateString('en-US', {weekday: 'short', month: 'short', day: 'numeric'})}</p>
                        <img src="${day.day.condition.icon}" alt="Weather icon" class="img-fluid mb-2">
                        <p class="mb-0">${Math.round(day.day.avgtemp_c)}°C / ${Math.round(day.day.avgtemp_f)}°F</p>
                        <p class="mb-0">Chance of rain: ${day.day.daily_chance_of_rain}%</p>
                    </div>
                </div>
            `).join('')}
        </div>
        ...
        weatherAlerts.innerHTML = `
            ...
            ${data.alerts.alert.map(alert => `
                <div class="alert alert-warning" role="alert">
                    ...
                    <hr>
                    <p class="mb-0">Effective: ${new Date(alert.effective).toLocaleString()}</p>
                    <p class="mb-0">Expires: ${new Date(alert.expires).toLocaleString()}</p>
                </div>
            `).join('')}
        `;
    } else {
...
function getAirQualityDescription(index) {
    ...
    return `${index} - ${descriptions[index - 1]}`;
}
